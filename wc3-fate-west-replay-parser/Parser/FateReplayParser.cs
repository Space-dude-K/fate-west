...
namespace wc3_fate_west_parser_replay_parser.Parser
{
    public class FateReplayParser
    {
        ...
        {
            logger.Info("[FW-PARSER] Loading file -> " + replayFilePath);
        ...
        public ReplayData ParseReplayData()
        {
            ...
            {
                logger.Error("[FW-PARSER] " + String.Format(ERROR_FILE_DOES_NOT_EXIST, _replayFilePath));
            ...
            {
                logger.Error("[FW-PARSER] " + String.Format(ERROR_FILE_EXTENSION_NOT_SUPPORTED, _replayFilePath));
        ...
        private DateTime? GetGameDateTimeFromFileName(string replayFileName)
        {
            try
            {
                ...
                // TODO 
                //DateTime gameDate = DateTime.Parse(spaceSeparatedFileName[1] + " " + spaceSeparatedFileName[2].Replace('-', ':'), CultureInfo.InvariantCulture);
                
                DateTime gameDate = DateTime.ParseExact(spaceSeparatedFileName[3] + spaceSeparatedFileName[4] + spaceSeparatedFileName[5], "MM_dd_yyyyhhmm", CultureInfo.InvariantCulture);
                Console.WriteLine("Parsed date: " + gameDate.ToString("dd-MM-yyyy HH:mm"));
