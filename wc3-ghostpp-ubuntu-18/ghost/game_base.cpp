...
void CBaseGame :: loop( )
{
	...
	CONSOLE_Print("[GHOST] - Starting game from thread: " + mystring);
...
bool CBaseGame :: Update( void *fd, void *send_fd )
{
	...
	// AUTO START SPAM
	if( !m_CountDownStarted && m_AutoStartPlayers != 0 && GetTime( ) - m_LastAutoStartTime >= 1000 )
	{
	...
	if( m_Players.empty( ) && ( m_GameLoading || m_GameLoaded ) )
	{
		if( !m_Saving )
		{
			...
			// Lobby export - update game status
			Lobby::UpdateGameStatusForLobby(m_RandomSeed, 2, m_GHost->m_DB);

			// Cancel lobby updater
...
void CBaseGame :: EventGameStarted( )
{
	...
	// Lobby export - change lobby status
	Lobby::UpdateGameStatusForLobby(m_RandomSeed, 1, m_GHost->m_DB);
