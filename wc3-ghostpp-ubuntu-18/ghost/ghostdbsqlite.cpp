...
// Lobby export - SQLite
bool CGHostDBSQLite::LobbyAdd(uint32_t lobbyId, uint32_t m_HostCounter, string m_CreatorServer, string m_GameName, int lobbyStatus, string dateTime, int lobbyType)
{
	bool Success = false;
	sqlite3_stmt* Statement;

	m_DB->Prepare("INSERT INTO lobbies ( lobbyId, gameCounter, realm, lobbyName, lobbyStatus, lobbyDateTime, lobbyType ) VALUES ( ?, ?, ?, ?, ?, ?, ? )", (void**)&Statement);

	if (Statement)
	{
		sqlite3_bind_int(Statement, 1, lobbyId);
		sqlite3_bind_int(Statement, 2, m_HostCounter);
		sqlite3_bind_text(Statement, 3, m_CreatorServer.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_text(Statement, 4, m_GameName.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_int(Statement, 5, lobbyStatus);
		sqlite3_bind_text(Statement, 6, dateTime.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_int(Statement, 7, lobbyType);
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error adding lobby [" + UTIL_ToString(lobbyId) + " : " + UTIL_ToString(m_HostCounter) + " : " + m_CreatorServer + " : " + m_GameName + "] - " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error adding lobby [" + UTIL_ToString(lobbyId) + " : " + UTIL_ToString(m_HostCounter) + " : " + m_CreatorServer + " : " + m_GameName + "] - " + m_DB->GetError());

	return Success;
}
bool CGHostDBSQLite::LobbySlotsInit(uint32_t lobbyId, int slotId, int playerId, int playerTeam, string playerName, int playerColour, int playerPing)
{
	bool Success = false;
	sqlite3_stmt* Statement;

	m_DB->Prepare("INSERT INTO slots ( lobbyID, slotId, playerId, playerTeam, playerName, playerColour, playerPing ) VALUES ( ?, ?, ?, ?, ?, ?, ? )", (void**)&Statement);

	if (Statement)
	{
		//CONSOLE_Print("Db add for slot: " + playerName);

		sqlite3_bind_int(Statement, 1, lobbyId);
		sqlite3_bind_int(Statement, 2, slotId);
		sqlite3_bind_int(Statement, 3, playerId);
		sqlite3_bind_int(Statement, 4, playerTeam);
		sqlite3_bind_text(Statement, 5, playerName.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_int(Statement, 6, playerColour);
		sqlite3_bind_int(Statement, 7, playerPing);
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error adding slot [" + UTIL_ToString(lobbyId) + " : " + UTIL_ToString(slotId) + " : " + UTIL_ToString(playerTeam) + " : " + playerName + "] - " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error adding slot [" + UTIL_ToString(lobbyId) + " : " + UTIL_ToString(slotId) + " : " + UTIL_ToString(playerId) + " : " + UTIL_ToString(playerTeam) + " : " +  playerName + "] - " + UTIL_ToString(playerColour) + " : " + UTIL_ToString(playerPing) + " : " + m_DB->GetError());

	return Success;
}
bool CGHostDBSQLite::LobbySlotUpdate(uint32_t nLobbyId, uint32_t nSlotId, int nPlayerId, int nPlayerTeam, string nPlayerName, int nPlayerColour, int nPlayerPing)
{
	bool Success = false;
	sqlite3_stmt* Statement = nullptr;

	m_DB->Prepare("UPDATE slots SET playerId=?, playerTeam=?, playerName=?, playerColour=?, playerPing=? WHERE lobbyId=? AND slotId=?", (void**)&Statement);

	if (Statement)
	{
		sqlite3_bind_int(Statement, 1, nPlayerId);
		sqlite3_bind_int(Statement, 2, nPlayerTeam);
		sqlite3_bind_text(Statement, 3, nPlayerName.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_int(Statement, 4, nPlayerColour);
		sqlite3_bind_int(Statement, 5, nPlayerPing);

		sqlite3_bind_int(Statement, 6, nLobbyId);
		sqlite3_bind_int(Statement, 7, nSlotId);
		//sqlite3_bind_text(Statement, 6, to_string(lobbyId).c_str(), -1, SQLITE_TRANSIENT);
		//(Statement, 7, to_string(slotId).c_str(), -1, SQLITE_TRANSIENT);
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error updating slot [" + to_string(nSlotId) + "] for lobby [" + to_string(nLobbyId) + "] " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error updating slot [" + to_string(nSlotId) + "] for lobby [" + to_string(nLobbyId) + "] " + m_DB->GetError());

	return Success;
}
bool CGHostDBSQLite::LobbyStatusUpdate(uint32_t nLobbyId, int gameStatus, string dateTime)
{
	bool Success = false;
	sqlite3_stmt* Statement = nullptr;

	m_DB->Prepare("UPDATE lobbies SET lobbyStatus=?, lobbyDateTime=? WHERE lobbyId=?", (void**)&Statement);

	if (Statement)
	{
		sqlite3_bind_int(Statement, 1, gameStatus);
		sqlite3_bind_text(Statement, 2, dateTime.c_str(), -1, SQLITE_TRANSIENT);
		sqlite3_bind_int(Statement, 3, nLobbyId);
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error updating game status [" + to_string(gameStatus) + "] for lobby [" + to_string(nLobbyId) + "] " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error updating game status [" + to_string(gameStatus) + "] for lobby [" + to_string(nLobbyId) + "] " + m_DB->GetError());

	return Success;
}
bool CGHostDBSQLite::LobbyDeleteAllLobbies()
{
	bool Success = false;
	sqlite3_stmt* Statement;
	m_DB->Prepare("DELETE FROM lobbies", (void**)&Statement);
	...
	if (Statement)
	{
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error removing lobbies - " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error removing lobbies - " + m_DB->GetError());

	return Success;
}
bool CGHostDBSQLite::LobbyDeleteAllSlots()
{
	bool Success = false;
	sqlite3_stmt* Statement;
	m_DB->Prepare("DELETE FROM slots", (void**)&Statement);
	...
	if (Statement)
	{
		...
		if (RC == SQLITE_DONE)
			...
			CONSOLE_Print("[SQLITE3] error removing slots - " + m_DB->GetError());

		m_DB->Finalize(Statement);
	}
	else
		CONSOLE_Print("[SQLITE3] prepare error removing slots - " + m_DB->GetError());
...
CCallableLobbySlotUpdate *CGHostDBSQLite :: ThreadedLobbySlotUpdate(uint32_t nLobbyId, uint32_t nSlotId, int nPlayerId, int nPlayerTeam, string nPlayerName, int nPlayerColour, int nPlayerPing)
{
	CCallableLobbySlotUpdate *Callable = new CCallableLobbySlotUpdate(nLobbyId, nSlotId, nPlayerId, nPlayerTeam, nPlayerName, nPlayerColour, nPlayerPing);
	Callable->SetResult(LobbySlotUpdate(nLobbyId, nSlotId, nPlayerId, nPlayerTeam, nPlayerName, nPlayerColour, nPlayerPing));
	Callable->SetReady(true);
	return Callable;
}
CCallableLobbyDeleteAllLobbies* CGHostDBSQLite::ThreadedLobbyDeleteAllLobbies()
{
	CCallableLobbyDeleteAllLobbies* Callable = new CCallableLobbyDeleteAllLobbies();
	Callable->SetResult(LobbyDeleteAllLobbies());
	Callable->SetReady(true);
	return Callable;
}
CCallableLobbyDeleteAllSlots* CGHostDBSQLite::ThreadedLobbyDeleteAllSlots()
{
	CCallableLobbyDeleteAllSlots* Callable = new CCallableLobbyDeleteAllSlots();
	Callable->SetResult(LobbyDeleteAllSlots());
	Callable->SetReady(true);
	return Callable;
}
CCallableLobbyStatusUpdate* CGHostDBSQLite::ThreadedLobbyStatusUpdate(uint32_t nLobbyId, int nGameStatus, string nDateTime)
{
	CCallableLobbyStatusUpdate* Callable = new CCallableLobbyStatusUpdate(nLobbyId, nGameStatus, nDateTime);
	Callable->SetResult(LobbyStatusUpdate(nLobbyId, nGameStatus, nDateTime));
	Callable->SetReady(true);
	return Callable;
}
CCallableLobbyAdd* CGHostDBSQLite::ThreadedLobbyAdd(uint32_t nLobbyId, uint32_t nHostCounter, string nCreatorServer, string nGameName, int nLobbyStatus, string nDateTime, int nLobbyType)
{
	CCallableLobbyAdd* Callable = new CCallableLobbyAdd(nLobbyId, nHostCounter, nCreatorServer, nGameName, nLobbyStatus, nDateTime, nLobbyType);
	Callable->SetResult(LobbyAdd(nLobbyId, nHostCounter, nCreatorServer, nGameName, nLobbyStatus, nDateTime, nLobbyType));
	Callable->SetReady(true);
	return Callable;
}
CCallableLobbySlotsInit* CGHostDBSQLite::ThreadedLobbySlotsInit(uint32_t nLobbyId, int nSlotId, int nPlayerId, int nPlayerTeam, string nPlayerName, int nPlayerColour, int nPlayerPing)
{
	CCallableLobbySlotsInit* Callable = new CCallableLobbySlotsInit(nLobbyId, nSlotId, nPlayerId, nPlayerTeam, nPlayerName, nPlayerColour, nPlayerPing);
	Callable->SetResult(LobbySlotsInit(nLobbyId, nSlotId, nPlayerId, nPlayerTeam, nPlayerName, nPlayerColour, nPlayerPing));
	Callable->SetReady(true);
	return Callable;
}
