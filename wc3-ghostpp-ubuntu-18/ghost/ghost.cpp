...
void SignalCatcher( int s )
{
	...
	// Lobby export - close DB writer thread.
	Lobby::RequestCancelForLobbyUpdater();

	Lobby::UpdateGameStatusForLobby(gGHost->m_CurrentGame->m_RandomSeed, 3, gGHost->m_DB);
	Lobby::DeleteAllSlots(gGHost->m_DB);
...
int main( int argc, char **argv )
{
	...
	ServerForDisc = CFG.GetString("bnet_server", string());
	...
	// Lobby export - Init
	Lobby::InitCancelTokenForLobbyUpdater();

	Lobby::DeleteAllSlots(gGHost->m_DB);

	// Lobby export - Init lobby updater.
	boost::thread(Lobby::RunLobbyUpdater, gGHost);
	...
	// Lobby export - delete all lobbys and slots
	//Lobby::DeleteAllLobbysAndSlots(gGHost->m_DB);
...
#endif
	m_lobbyExportDelay = CFG->GetInt("lobbyExport_updateDelayInSeconds", 30);
	m_Language = NULL;
	m_Exiting = false;
	m_ExitingNice = false;
	m_Enabled = true;
...
void CGHost :: CreateGame( CMap *map, unsigned char gameState, bool saveGame, string gameName, string ownerName, string creatorName, string creatorServer, bool whisper )
{
	...
	// Lobby export - Init lobby.
	Lobby::InitLobby(m_CurrentGame->m_RandomSeed, m_CurrentGame->m_HostCounter, ServerForDisc, m_CurrentGame->GetGameName(), 0, gameState, m_CurrentGame->m_GHost->m_DB);

	// Lobby export - Init slots.
	Lobby::InitSlotsForLobby(m_CurrentGame->m_RandomSeed, m_CurrentGame->m_Slots, m_CurrentGame->m_GHost->m_DB);

	// Lobby export- Update status.
	Lobby::UpdateGameStatusForLobby(m_CurrentGame->m_RandomSeed, 0, m_CurrentGame->m_GHost->m_DB);

	
	//boost::thread(Lobby::RunLobbyUpdater, m_CurrentGame, m_lobbyExportDelay);
}
